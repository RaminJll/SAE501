...
router.get("/allProduitsLaitiers", async (req, res, next) => {
  ...
  try {
    const urls = [
      ...
      { 
        url: "https://world.openfoodfacts.org/api/v2/search", 
        params: { 
          categories_tags_fr: 'yaourts', 
          fields: fields 
        }
      }
    ];

    const excludedTag = "chocolates";

    // Récupérer les données des deux URLs
    const responses = await Promise.all(
      urls.map(config => axios.get(config.url, { params: config.params }))
    );

    const filteredResults = responses.map(response => 
      response.data.products.filter(product => 
        !product.categories_tags || 
        !product.categories_tags.some(tag => tag.includes(excludedTag)) ||
